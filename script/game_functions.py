        ...
        if settings_flag:
            ...
            if action["action"] == "defence":
                    ...
                    if pressed[0]:
                        if continue_btn.on_hovered(pygame.mouse.get_pos()):
                            if action["player"] == player1:
                                game_data[player1]["player_board"] = board.board.copy()
                                board.fill_board()
                ...
                if game_data[action['player']]["count_steps"] == 0 and action["player"] == player2:
                    ...
                    if pressed[0]:
                        if continue_btn.on_hovered(pygame.mouse.get_pos()):
                            update_leaderboard(game_data, player1, player2)
                            transition()
                ...
                if active_box == 1:
                    if event.key == pygame.K_RETURN:
                        if text_input_player1:
                            if len(text_input_player1) < MIN_LIMIT_SYMBOLS:
                                warning_window(f"Минимум {MIN_LIMIT_SYMBOLS} символа в имени")
                            else:
                                active_box = 2
...
def update_leaderboard(game_data, player1, player2):
    leaderboard_data = get_leader_data()
    nicknames = [data["nickname"] for data in leaderboard_data]
    if player1 not in nicknames:
        leaderboard_data.append({"nickname": player1, "max_score": game_data[player1]["score"], "games_count": 1})
    if player2 not in nicknames:
        leaderboard_data.append({"nickname": player2, "max_score": game_data[player2]["score"], "games_count": 1})
    else:
        for data in leaderboard_data:
            if data["nickname"] == player1:
                if game_data[player1]["score"] > data["max_score"]:
                    data["max_score"] = game_data[player1]["score"]
                data["games_count"] += 1
            if data["nickname"] == player2:
                if game_data[player2]["score"] > data["max_score"]:
                    data["max_score"] = game_data[player2]["score"]
                data["games_count"] += 1
